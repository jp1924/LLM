...
자주 하는 학습 방식을 코드로 굳혀둔 것이고, 매번 만들던 1회용 코드를 최대한 줄이고 GPU 처리량을 낭비 없이 효율적으로 사용하는데 초점이 맞춰져 있다.
...
학습/평가
- `data_preprocessor_type`(SFT): sft / pretrain / dpo 선택
...
SFT 스크립트 하나로 sft와 pretrain을 둘 다 돌린다. `data_preprocessor_type`만 sft / pretrain으로 바꾸면 된다.
...
### 파일 구조: 분리 → 통합
`args.py`/`collator.py`로 쪼갰더니 창만 늘고 한눈에 안 들어왔다. `main.py` 통합으로 바꿨다. lr scheduler/optimizer 전용 `optimization.py`도 한때 있었으나 지금은 제거했다.

### 데이터 전처리: 1회용 스크립트 → builder script → 재설계 중
- 초기엔 학습할 때마다 이상치 제거, split 분할용 1회용 코드를 매번 짰다.
- datasets 3.6.0까지는 builder script를 적극 활용해 학습 field를 고정 컬럼으로 정리했다. 1회용 코드가 builder script로 쌓이면서 재생산성이 생겼다.
- datasets 4.0.0에서 builder script가 폐지돼 지금은 대안을 찾는 중이다. 별도 전처리 스크립트는 TRL 파이프라인과 기능이 겹치지만, TRL은 기능이 제한적이라 불편해서 직접 만든 preprocessor를 유지한다. 목표는 그대로 "1회용 작업 최소화"다.
